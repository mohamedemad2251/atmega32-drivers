...
static u8 GLOB_u8SPIDataRecieved = TRUE;
#elif(SPI_SELECT == SPI_MASTER)
static u8 * GLOB_u8SPISendData;	//The data to be sent (no matter how big)
static u8 GLOB_u8SPIDataPending = FALSE;	//A data word is to be sent and not to be interrupted (pending)
static u8 GLOB_u8SPIDataSent = FALSE;	//Data sending done
#endif
#endif
...
STD_Type MCAL_SPI_u8Init(void)
{
	...
	#if(SPI_SELECT == SPI_MASTER)
	MCAL_DIO_u8SetPinValue(SS_PORT,SS_PIN,DIO_HIGH);
	#endif
	#if(SPI_INTERRUPT == SPI_INTERRUPT_ON)
	SET_BIT(SREG,I_BIT);
	#endif
...
{
	STD_Type LOC_u8ReturnValue = E_NOT_OK;
	if(GLOB_u8SPIDataRecieved == FALSE)
	{
...
{
	STD_Type LOC_u8ReturnValue = E_NOT_OK;
	if(LOC_Pu8Data && (LOC_Pu8Ptr != NULL) && GLOB_u8SPIDataPending == FALSE)
	{
		GLOB_u8SPISendData = LOC_Pu8Data;
		GLOB_PvoidSPIGptr = LOC_Pu8Ptr;
		MCAL_DIO_u8SetPinValue(SS_PORT,SS_PIN,DIO_LOW);
		MCAL_SPI_u8SendData(GLOB_u8SPISendData[ZERO]);
		GLOB_u8SPISendData++;
		GLOB_u8SPIDataPending = TRUE;
...
{
	#if(SPI_SELECT == SPI_SLAVE)
	if(GLOB_u8SPIDataRecieved == TRUE)
	{
		MCAL_SPI_u8RecieveData(&GLOB_u8SPIGetData);
		GLOB_u8SPIDataRecieved = FALSE;
	}
	#elif(SPI_SELECT == SPI_MASTER)
	if(*GLOB_u8SPISendData && GLOB_u8SPIDataPending)
	{
		MCAL_DIO_u8SetPinValue(SS_PORT,SS_PIN,DIO_LOW);
	...
	{
		GLOB_u8SPIDataPending = FALSE;
		GLOB_u8SPIDataSent = TRUE;
		MCAL_DIO_u8SetPinValue(SS_PORT,SS_PIN,DIO_HIGH);
	}
	#endif
