...
#if(TIMER0_INTERRUPT == TIMER_INTERRUPT_ON && (TIMER0_MODE == PWM || TIMER0_MODE == PHASE_CORRECT))
...
#if(TIMER1_INTERRUPT == TIMER_INTERRUPT_ON && (TIMER1_MODE == PWM || TIMER1_MODE == PHASE_CORRECT))
...
#if(TIMER2_INTERRUPT == TIMER_INTERRUPT_ON && (TIMER2_MODE == PWM || TIMER2_MODE == PHASE_CORRECT))
...
#define WGM1_23_MODE 0b00010000
#endif
...
#elif( (TIMER0_INTERRUPT == TIMER_INTERRUPT_ON) )
#if( TIMER0_MODE == NORMAL )
...
STD_Type MCAL_TIMER_u8Init(u8 LOC_u8TimerNo)
{
	...
	if(LOC_u8TimerNo <= TIMER2)
	{
		switch(LOC_u8TimerNo)
		{
			case TIMER0:
			TCCR0 = TCCR0_COMMAND;
			#if( (TIMER0_MODE == PWM || TIMER0_MODE == PHASE_CORRECT) && OC0_MODE != OC0_OFF)
			...
			#if( (TIMER1_MODE == PWM || TIMER1_MODE == PHASE_CORRECT) && OC1A_MODE != OC1A_OFF)
			LOC_u8ReturnValue = MCAL_DIO_u8SetPinDirection(OC1A_PORT,OC1A_PIN,DIO_OUTPUT);
			#endif
			#if( (TIMER1_MODE == PWM || TIMER1_MODE == PHASE_CORRECT) && OC1B_MODE != OC1B_OFF)
			LOC_u8ReturnValue = MCAL_DIO_u8SetPinDirection(OC1B_PORT,OC1B_PIN,DIO_OUTPUT);
			#endif
			break;
			case TIMER2:
			TCCR2 = TCCR2_COMMAND;
			#if( (TIMER2_MODE == PWM || TIMER2_MODE == PHASE_CORRECT) && OC2_MODE != OC2_OFF)
...
STD_Type MCAL_TIMER_u8CallbackMilliFun(u8 LOC_u8TimerNo,u32 LOC_u32MilliSeconds,void (*LOC_Pu8Ptr)(void))
{
	...
	if(LOC_Pu8Ptr != NULL && LOC_u8TimerNo <= TIMER2)
	{
		switch(LOC_u8TimerNo)
		{
			...
			HAL_LCD_u8WriteInteger(GLOB_u32TIMER1OverflowNo);
			GLOB_voidTIMER1Gptr = LOC_Pu8Ptr;
			#endif
			#if( TIMER1_MODE == NORMAL)
			if(LOC_u32TotalTicks > TIMER1_TICKS_MAX)
			{
				TCNT1H = (u8)(((TIMER1_TICKS_MAX)-(u16)(((LOC_u32TotalTicks)%(TIMER1_TICKS_MAX))))>>8);
				TCNT1L = (u8)(((TIMER1_TICKS_MAX)-(u16)(((LOC_u32TotalTicks)%(TIMER1_TICKS_MAX)))));				//TCNT1 = (TIMER1_TICKS_MAX)-(u16)(((LOC_u32TotalTicks)%(TIMER1_TICKS_MAX))*(TIMER1_TICKS_MAX));
			}
			else
			{
			...
			{
				TCNT2 = (TIMER2_TICKS_MAX)-(u8)(((LOC_u32TotalTicks)%(TIMER2_TICKS_MAX)));
			}
			else
			{
...
STD_Type MCAL_TIMER_u8CallbackMicroFun(u8 LOC_u8TimerNo,u32 LOC_u32MicroSeconds,void (*LOC_Pu8Ptr)(void))
{
	...
	if(LOC_Pu8Ptr != NULL && LOC_u8TimerNo <= TIMER2)
	{
		switch(LOC_u8TimerNo)
		{
			...
			if(LOC_u32TotalTicks > TIMER1_TICKS_MAX)
			{
				TCNT1 = (TIMER1_TICKS_MAX)-(u16)(((LOC_u32TotalTicks)%(TIMER1_TICKS_MAX)));
			}
			else
			{
...
STD_Type MCAL_TIMER_u8SetPWM(u8 LOC_u8TimerNo,u8 LOC_u8Percentage)
{
	...
	if(LOC_u8TimerNo <= TIMER2 && LOC_u8Percentage >= ZERO && LOC_u8Percentage <= PERCENT)
	{
		switch(LOC_u8TimerNo)
		{
			...
			LOC_u8ReturnValue = E_OK;
			#elif( OC0_MODE == OC0_SET)
			OCR0 = TIMER0_TICKS_MAX - (u8)((u32)LOC_u8Percentage*(TIMER0_TICKS_MAX)/PERCENT);
			LOC_u8ReturnValue = E_OK;
			#endif
			break;
			case TIMER2:
			...
			LOC_u8ReturnValue = E_OK;
			#elif( OC2_MODE == OC2_SET)
			OCR2 = TIMER2_TICKS_MAX - (u8)((u32)LOC_u8Percentage*(TIMER2_TICKS_MAX)/PERCENT);
			LOC_u8ReturnValue = E_OK;
			#endif
			break;
		}
